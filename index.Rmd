...
```{r echo = FALSE}
dygraph(OHLC(TSLA), main = "History of Tesla's Stock Prices", ylab = "Stock Price ($USD)", xlab = "Year") %>% 
  dyEvent("2013-05-01", "First quarterly profit", labelLoc="bottom") %>% 
  dyEvent("2015-09-01", "Model X released", labelLoc="bottom") %>% 
  dyEvent("2016-01-01", "Inability to deliver 4000 Model X's", labelLoc="bottom") %>%
  dyEvent("2017-07-07", "Model 3 released", labelLoc="bottom") %>% 
  dyEvent("2018-06-01", "Increased production of Model 3", labelLoc="bottom") %>% 
...
ts_info(tesla_ts)
ts_tesla <- ts(tesla_ts,start=decimal_date(as.Date("2010-06-30")),freq=253)
plot(decompose(ts_tesla))
...
pforecast <- forecast1$mean # element mean
